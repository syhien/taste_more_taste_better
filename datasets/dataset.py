...
class Crowd(data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.method == "train":
            ...
            try:
                inpaint = (
                    Image.open(
                        os.path.join(self.inpaint_path, name, f"{name}.jpg")
                    ).convert("RGB")
                    if os.path.exists(
                        os.path.join(self.inpaint_path, name, f"{name}.jpg")
                    )
                ...
                inpaint = None
            return self.train_transform_density_map(img, den_map, label, inpaint, name)
    ...
    def train_transform_density_map(self, img, den_map, label, inpaint=None, name=None):
        wd, ht = img.size
        if random.random() > 0.88:
            img = img.convert("L").convert("RGB")
            inpaint = inpaint.convert("L").convert("RGB") if inpaint else None
        ...
        if min(wdd, htt) >= self.c_size:
            ...
            inpaint = inpaint.resize((wd, ht)) if inpaint else None
            den_map = cv2.resize(
                den_map[:, :], (wd, ht), interpolation=cv2.INTER_CUBIC
            ) / (re_size**2)
        ...
        inpaint = F.crop(inpaint, i, j, h, w) if inpaint else None
        den_map = den_map[i : (i + h), j : (j + w)]
        den_map = den_map.reshape(
            [h // self.d_ratio, self.d_ratio, w // self.d_ratio, self.d_ratio]
        ).sum(axis=(1, 3))

        if random.random() > 0.5:
            img = F.hflip(img)
            inpaint = F.hflip(inpaint) if inpaint else None
            den_map = np.fliplr(den_map)

        return (
            self.w_transform(img),
            self.s_transform(img),
            torch.from_numpy(den_map.copy()).float().unsqueeze(0),
            label,
            name,
            self.w_transform(inpaint if inpaint else img),
            self.s_transform(inpaint if inpaint else img),
            inpaint is not None,
        )
